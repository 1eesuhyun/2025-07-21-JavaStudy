/*   
 *   문자열 검색 기능
 *   
 *   문자열
 *   ----- 자바
 *   String => 데이터형 / 클래스형 (기능=>메소드)
 *    => 사용법
 *       String 변수명 ="문자"
 *                     ---- 제한이 없다
 *       예) 회원정보
 *          ------  id, pwd, name, email, address, phone, post, content, birthday
 *          게시판 정보
 *           no name subject content pwd regdate hit
 *        -> 제공하는 기능
 *           메소드
 *          변환
 *           toUpperCase() : 대문자 변환
 *           toLowerCase() : 소문자 변환
 *           ***** valueOf() : 모든 데이터형을 문자로 변환
 *                  웹/ 윈도우 => 모든 데이터형 String
 *                  valueOf(10) => "10" 
 *           -------------------------(사용빈도가 없다) 영어로 검색할때 주로 쓰인다
 *          비교
 *           equals()     => 실제 문자열 비교
 *           contains()   => 포함 문자열
 *           startsWith() => 시작 문자열
 *           endsWith()   => 끝 문자열 => 확장자
 *          검색
 *           Hello Java!!
 *           0123456789
 *           indexOf("a") => 7
 *           indexOf("Java") => 6
 *            => 앞에서 부터 찾기
 *           lastIndexOf("a") => 9
 *            => 뒤에서 부터 찾기
 *          제어
 *           substring(1,5) : 문자 자르기
 *                       ↑=end-1
 *           split() : 문자별 자르기
 *           trim()  : 공백문자 제거
 */
// substring 
import java.io.*;
import java.util.*;
public class 문자열_1 {

	public static void main(String[] args)throws Exception {
		// TODO Auto-generated method stub
		String msg="Hello Java";
		// 1. 문자의 갯수 => " " 공백도 문자
		// => length();
		int leng=msg.length();
		System.out.println(("문자 갯수: ")+leng+"개\n");
		// 2. 자르기
		String s=msg.substring(6);
		System.out.println(s);
		s=msg.substring(0, 5);
		// substring(beginIndex,endIndex);
	    //                        ↑ endIndex-1
		System.out.println(s);
		String name="문자열_1.java";
		//           0 123456789
		String ext=name.substring(name.indexOf(".")+1);
		System.out.println(ext);
		
//		File dir=new File("c://javaDev");
//		File[] lists=dir.listFiles();
//		// ./ ..
//		for(File f:lists)
//		{
//			if(f.isFile())
//			{
//				String ss=f.getName();
//				//System.out.println(ss);
//				System.out.println(ss.substring(ss.lastIndexOf(".")+1));
//			}
//		}
		
//		String msg1="                                                               Hello Java!!                                                ";
//		System.out.println(msg1.length());
//		String ss=msg1.trim();
//		System.out.println(ss.length());
		// trim() => 공백문자 제거 => 좌우만 제거 가능(가운데 있는 공백 제거 불가능)
		// trim() ex).. 로그인, 검색, 아이디 중복체크
		// 액션 | ㅁㅁㅁ | ㅁㅁㅁㅁ
		// 서울시 마포구 서교동
		System.out.println();
		String msg2="Java Oracle HTML CSS JavaScript"
					+"JSP SpringFramework VueJS"
					+"SpringBoot Git GitAction Docker Docker-Compose"
					+"Jenkins React Next TypeScript MSA Chattint NoedJS";
		String[] datas=msg2.split(" ");
		Scanner scan=new Scanner(System.in);
		System.out.print("단어 입력: ");
		String d=scan.next();
		for(String data:datas)
		{
			//System.out.println(data);
			//if(data.startsWith(d)) // 시작하는 단어 찾기
			//if(data.endsWith(d)) // 끝자리 단어 찾기
			//if(data.contains(d)) // 포함된 단어 찾기
			//if(data.equals(d))  //=> 대소문자 구분
			if(data.equalsIgnoreCase(d)) // => 대소문자 구분X
			{
				System.out.println(data);
			}
		}
	}

}
