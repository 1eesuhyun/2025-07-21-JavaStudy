 ...
 *                           1. 데이터를 보호할 목적
 *                           2. 경계선
 *                           3. 메소드와 변수를 한개로 묶어서 외부에서 직접 접근하지 못하게 만든다 => getter(읽기) / setter(쓰기) => lombok
 *                           4. 접근 지정어 사용법
 *                           5. 웹에서 주로 캡슐화 사용
 *       => 상속
 *          기존의 변수와 메소드를 재사용 => 확장
 ...
 *                                       객체.변수
 *                    객체=new 클래스명(); (필요한 데이터를 저장하면 '인스턴스')
 *                    클래스명 객체=new 클래스명();
 *       class 설계
 *       ---------
 *       구조
 ...
 *          }
 *       2) 인스턴스 변수
 *          - 클래스 내부 블록에 선언 => 클래스 위나 아래에 선언이 가능
 *          - 객체 생성시에 메모리에 저장 => new를 사용할때 메모리에 저장
 *                                  ---- 새로운 메모리 생성
 *          - 초기값은 지정하지 않는 경우 => 자동으로 기본값이 설정됨
 ...
 *               인스턴스 변수 : 객체마다 따로 메모리가 생성, 기본값이 있음
 *               --------- 객체.변수
 *               정적변수 => 클래스 변수, 공유변수
 *                         --------
 *                         클래스명.변수
 ...
 *                         => 데이터 묶음, 오라클에 저장된 데이터
 *             ** 초기화 여부
 *                - 지역변수    : 필수, 직접 대입, 연산 결과, 기본값은 없음
 *                - 인스턴스변수 : 선택, 선언시 대입, 생성자에서 대입, 기본값 존재
 ...
 *         저장되는 순서
 *         ---------
 *         기본값 -> 명시적 초기화 -> 정적 블록 -> 인스턴스 블록 -> 생성자
 *         인스턴스 변수 : 반드시 생성자를 호출 => 생성자 초기화
 *         정적 변수 : 자동 저장 => static {}
 *         ** 변수는 초기값이 있어야 메모리에 저장이 가능 => 프로그램에서 사용이
 *         
 *         생성자
 *           => 객체가 생성이 될때 호출되는 메소드 형태의 코드 블록
 *                                   ---------
 ...
 *                 => 생성자가 없는 경우는 컴파일러가 자동으로 기본생성자 대입
 *                 => 기본 생선자 => 매개변수가 없는 생성자
 *              => 생성자의 종류
