 ...
 *           {
 *           }
 *        2. 접근지정어   : 변수 / 메소드 / 클래스 / 생성자
 *                     => 접근 권한을 지정해서 사용
 *                        private(인스턴스 변수(데이터 보호)) < default < protected < public(다른 클래스와 통신이 가능) 클래스 / 생성자 / 메소드
 *           - private   : 같은 클래스 내부에서만 접근이 가능 => 자신 클래스안에서만 사용 가능
 *                       : 인스턴스 변수 => 은닉화 => 캡슐화
 *           - default   : 같은 폴더까지만 접근이 가능
 *           - protected : 같은 폴더까지만 접근이 가능, 다른 폴더인 경우에 상속이 있어야 접근 가능
 *           - public    : 모든 클래스에 접근 가능
 *         --------------------------------------------------------
 *                     자신의 클래스 | 같은 폴더 | 상속 받은 경우 | 모든 클래스
 *         --------------------------------------------------------
 *         private         o          x          x          x
 *         --------------------------------------------------------
 *         default         o          o          x          x
 ...
 *        [접근지정어] 리턴형 메소드명(매개변수..)
 *        {
 *        	 return 값 ==> 생략 void => 생략이 되면 자동으로 컴파일러가 실행
 *        }    |
 *           메소드 종료
