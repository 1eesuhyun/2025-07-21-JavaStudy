 ...
 *               |    데이터 전송
 *               |
 *       전송받은 데이터를 메소드를 통해서 브라우저에 전송
 *       ----------- 브라우저, 핸드폰, 윈도우
 *       3) 유지보수 용이
 *       4) 프로그램을 구조화(기능별로 분리가 가능)
 *       ** 한가지 기능만 수행 ex) 예약프로그램을 짜면 예약만 가능 다른 프로그램을 짜려면 다시 코드를 짜야함
 *       ** 메소드
 *          - 사용자 정의 메소드
 *          - 라이브러리 메소드 => 암기
 ...
 *            public : 모든 클래스에서 접근 가능(패키지와 관련 없음)
 *            
 *            데이터를 저장하는 일반변수 : private(웹)
 *            메소드 / 클래스 / 인터페이스 : public => 연결
 *            메소드 ㅣ public
 *            *** 인터페이스 : 모든 변수가 public
 *                -------- 선언되는 상수
 *                
