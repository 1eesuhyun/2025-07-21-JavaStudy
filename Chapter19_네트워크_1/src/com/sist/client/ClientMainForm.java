...
implements ActionListener,MouseListener,Runnable
{
    ...
    public ClientMainForm()
    {
    	...
    	wr.b5.addActionListener(this); // 정보보기
	...
	public void connection(String id,String name,String address)
	{
		try
		{
			...
			// 송수신
			// 서버에서 데이터 읽기 : 응답 => HttpServletResponse(Buffered)
			in=new BufferedReader(new InputStreamReader(s.getInputStream()));
			// InputStreamReader : 보조 스트림 | byte => char로 변환
			// 서버로 데이터 보내기 : 요청 => HttpServletRequest
			out=s.getOutputStream();
			// 서버로 로그인 요청
			out.write((Function.LOGIN+"|"+id+"|"+name+"|"+address+"\n").getBytes());
			// readLine로 읽을때는 반드시 마지막에 '\n'이 있어야 문장이 끝난다
		}catch(Exception ex) {}
		// 서버로부터 데이터를 읽기 시작
	...
	public void mouseClicked(MouseEvent e) {
		...
		{
			selectRow=wr.table2.getSelectedRow(); // 버튼 클릭
			String id=wr.model2.getValueAt(selectRow, 0).toString(); // 누구의 ID 인지
			if(id.equals(myId)) // 내 ID면 비활성화
			{
	   ...
	   public void run() {
		...
		// 서버에서 데이터 읽기
		   try
		   {
			   while(true)
			   {
				   String msg=in.readLine(); // 서버에서 들어오는 값을 받을 수 있음
