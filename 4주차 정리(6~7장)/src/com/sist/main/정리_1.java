 ...
 *                 }
 *                 
 *                 A a=new A(); -> 객체 1개 생성 'A'가 객체
 *                 a.aaa();
 *                 b.bbb(); 
 *                 ----------------------------------
 *                 new A().aaa() 
 *                 new A().bbb() -> 서로다른 객체가 2개 생성
 *         class ClassName
 *         {
 *           ---------------
 *           1. 변수
 *              - 인스턴스 변수 : 객체마다 가지고 있는 변수
 *                = new를 사용시에 메모리가 따로 생성
 *              - 정적변수 : 한개의 메모리만 사용
 *                = 생성된 모든 객체가 공유한는 변수
 *                = static
 ...
 *              = 객체 생성시에 변수값을 초기화
 *              = 시작과 동시에 처리하는 기능
 *                ex) 자동 로그인 / 화면 UI / 서버구동
 *              = 기본 성성자(매개변수가 없는 생성자)
 *                ** 생성자를 사용하지 않는 경우에 자동으로 추가
 *              = 특징
 *                1) 클래스명과 동일
 *                2) 리턴형이 없다
 *                3) 오버로딩 => 여러개의 생성자가 필요한 경우
 *                   => 구분 : 매개변수
 ...
 *                  매개변수
 *                   없는 경우 : 자체 처리 => 출력
 *                   있는 경우 : 사용자 요청, ex) 장바구니의 총합을 구하라 => id를 넘겨서 그 아이디 장바구니에 들어있는 총합을 구함
 *                           - 여러개인(3개이상) 경우 => 클래스 / 배열
 *                           - 사용자가 요청한 값이 몇개인지 모르는 경우 => 가변 매개변수 -- 데이터형이 여러개면 Object...
 *                           - Object가 데이터형 중에 가장 큰 데이터형
 *                             Object o=1
 *                             o=1.0
 *                             o='a'
 *                             o="aaa"
